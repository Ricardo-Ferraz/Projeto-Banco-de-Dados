...
public class JanelaSaida extends JFrame implements ActionListener {
        ...
        if(e.getSource() == b1){ //Bloco G
            ...
            if(opcao == JOptionPane.OK_OPTION){
                try{
                    ...
                    saidaDAO.add(new Saida(new Date(data.getTime().getYear(),data.getTime().getMonth(),data.get(Calendar.DAY_OF_MONTH)), data.getTime().getHours()+":"+data.getTime().getMinutes()+":"+data.getTime().getSeconds(),
                            aux, new Estacionamento(1,"Bloco G",200)));
                    JOptionPane.showMessageDialog(null, "Saida Liberada!","Sucesso!", JOptionPane.INFORMATION_MESSAGE);
                }catch (Exception ex){
                    ex.printStackTrace();
                }
            }
